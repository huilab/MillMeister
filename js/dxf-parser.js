 ...
 */
// Accepts a dxf file and returns an object containing the data organized by layer
// using the function getGeo(dxf)
//
// Modified from dxf-parser:
 ...
 */
function getCircles(sections, options){
 ...
 */
function getParameters(sections) {
 ...
 */
function getLayersByEntities(sections, ents){
...
function getGeo(dxf){
	...
	for(var key in p){
		if(p.hasOwnProperty(key)){
			...
			colorMap.push(layerData);
		}
	}

	for(var i=0; i<dxf.entities.length; ++i) {
		var placed = false;
		//look for a layer created for this entity
		for(var j=0;j<dxfGeo.layers.length;++j) {
			if(dxfGeo.layers[j].name == dxf.entities[i].layer){
				dxfGeo.layers[j].contents.push(dxf.entities[i]);
				placed = true;
				break;
			}
			else {
				//not a match, check the next layer name
			}
		}
		if(placed == false) {
			var layer = {
				name: dxf.entities[i].layer,
				color:0,
				type: dxf.entities[i].type,
				contents: []
			};
			for(var k=0;k<colorMap.length;++k) {
				if(colorMap[k].name == layer.name){
					layer.color = colorMap[k].color;
				}
			}
			layer.contents.push(dxf.entities[i]);
			dxfGeo.layers.push(layer);
		}
	}
	return dxfGeo;
};